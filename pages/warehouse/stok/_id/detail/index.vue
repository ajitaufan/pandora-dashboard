...
<div v-if="datash" class="wrapper">
  <div class="animated fadeIn">
    ...
    <div> 
      ...
      <h6>Terdapat di gudang {{datash.gudang.nama}}</h6>
      <p align='right'>
        <nuxt-link class="btn btn-sm btn-success btn" :to="{path: './detail/add-barang-stok'}">
      ...
      <table class="table table-hover">
        <thead>
          <tr class="table-active">
            <th scope="col">ID Stok Detail</th>
        ...
        <tbody v-if="datash">
          <tr v-for="stok in datash.stokDetail" :key="stok.id">
            ...
            <td>
              <center>
                <nuxt-link class="btn btn-sm btn-warning" style="margin-right:3px" :to="{name: 'warehouse-stok-id-detail-barangid-ubahKuantitas', params: {idStok: stokh.id, idBrg: stok.id}}">Ubah Kuantitas</nuxt-link>
              </center>
            </td>
          </tr>
        </tbody>
    ...
    axios
      .get(
        process.env.myapi +
          ...
          this.stokh.id +
          "){ nomor jenis tanggal gudang{ id nama } stokDetail{ id_barang id_stok_header id satuan kuantitas barang{ id nama sku } } } }"
      )
